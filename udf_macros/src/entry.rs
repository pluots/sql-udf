...
fn make_basic_fns(parsed: &ItemImpl, impl_for_name: &Ident) -> proc_macro2::TokenStream {
    ...
    let content = match make_type_list().iter().find(|x| x.type_ == *impl_item_type) {
        Some(t) => make_basic_fns_content(t, impl_for_name),
        None => {
            let emsg = format!(
...
fn make_proc_str_fn(
    ...
    // between our caller and callee
    let fn_title = if nullable {
        quote! { udf::ffi::wrapper::wrap_process_str_null::<#dstruct_ident, _> }
    } else {
        quote! { udf::ffi::wrapper::wrap_process_str::<#dstruct_ident, _> }
    };
...
fn make_add_fn(dstruct_ident: &Ident, fn_name: Ident) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        pub unsafe extern "C" fn #fn_name (
            ...
            unsafe {
                udf::ffi::wrapper::wrap_add::<#dstruct_ident>(initid, args, is_null, error)
            }
        }
    }
}
...
fn make_clear_fn(dstruct_ident: &Ident, fn_name: Ident) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        pub unsafe extern "C" fn #fn_name (
            ...
            unsafe {
                udf::ffi::wrapper::wrap_clear::<#dstruct_ident>(initid, is_null, error)
            }
        }
    }
}
...
fn make_remove_fn(dstruct_ident: &Ident, fn_name: Ident) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        pub unsafe extern "C" fn #fn_name (
            ...
            unsafe {
                udf::ffi::wrapper::wrap_remove::<#dstruct_ident>(initid, args, is_null, error)
            }
        }
    }
}
